...
class MessengerStore extends EntitiesStore {
  getChatReference = (chatId) => {
    return firebase.database().ref(CHATS_REFERENCE).child(chatId).child(MESSAGES_REFERENCE)
  }
  ...
  @action setTimestamp = (payload) => {
    const timestamps = Object.values(payload)
      .filter((x) => x.lastMessage)
      .map((x) => x.lastMessage.timestamp)
  ...
  @action subscribeOnChats = () => {
    ...
    this.currentUserChatsReference.orderByChild('lastMessage/timestamp').limitToLast(1).on('value', callback)
  }
  ...
  @action fetchChats = async () => {
    ...
    let ref = this.currentUserChatsReference.orderByChild('lastMessage/timestamp').limitToLast(chunkLength)
  ...
  getChatWith = async (userId, user) => {
    ...
    return await this.currentUserChatsReference.orderByChild('userId').equalTo(userId).once('value').then(callback)
  }
  ...
  @action subscribeOnMessages = (chatId) => {
    ...
    this.getChatReference(chatId).limitToLast(1).on('child_added', callback)
  }
  ...
  @action fetchMessages = (chatId) => {
    ...
    let ref = this.getChatReference(chatId).orderByKey().limitToLast(chunkLength)
  ...
  @action convertFetchedMessages = async (payload) => {
    const messagesPromises = Object.entries(payload).map(async ([key, message]) => {
      message.key = key
      message.userId = message.user
